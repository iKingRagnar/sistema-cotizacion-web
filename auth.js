...
async function attemptLogin(username, password) {
  /* NULL en activo (filas antiguas / migraciones) debe tratarse como cuenta habilitada (DEFAULT 1). */
  const u = await db.getOne(
    'SELECT * FROM app_users WHERE lower(username) = lower(?) AND COALESCE(activo, 1) = 1',
