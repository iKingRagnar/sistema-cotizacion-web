...
<body>
  <div class="tpl-toolbar">
    <div style="background:#ff0066;color:#fff;padding:4px 12px;font-weight:900;font-size:13px;letter-spacing:1px;border-radius:4px;align-self:center;margin-right:auto">FLYER VECTORIAL v280 ACTIVO · modo: ${isSingle ? 'SINGLE' : 'PAIR'} · debug: ${debugMode ? 'ON' : 'OFF'}</div>
...
<style>
  * { box-sizing: border-box; }
  ...
  /* Layout 2 columnas tipo ejemplo1: izquierda contenido | derecha foto industrial recortada */
  .uv-hero { display: flex; align-items: stretch; }
  .uv-hero-left { flex: 1 1 55%; min-width: 0; }
  .uv-hero-right { flex: 0 0 45%; position: relative; overflow: hidden; align-self: stretch; min-height: 240px; }
  /* La imagen ejemplo1.jpeg se escala al doble del ancho del wrapper (200%) y se posiciona alineada a su derecha,
     mostrando SOLO la mitad derecha de la imagen (donde está la máquina CNC con líquido refrigerante).
     Verticalmente solo se ven los primeros pixels (header de ejemplo1 con la máquina). */
  .uv-hero-fondo { display: block; width: 100%; height: 100%; object-fit: cover; object-position: center; user-select: none; pointer-events: none; }
