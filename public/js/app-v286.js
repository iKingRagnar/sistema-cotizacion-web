...
<body>
  ...
  function previewMaquinaUniversal(m, opts) {
    ...
    // En modo PAIR sin compañera del catálogo: si Luis subió una 2da foto, duplica m con esa foto.
    if (modo === 'pair' && !companion && m && m.imagen_ensamble_url) {
      companion = Object.assign({}, m, { imagen_pieza_url: m.imagen_ensamble_url, imagen_ensamble_url: null });
    }
    // FORZAR PAIR: si modo=pair pero no hay companion (ni pareja ni foto 2), duplicar m.
    // Garantiza que SIEMPRE se vean 2 cards en modo PAIR.
    if (modo === 'pair' && !companion && m) {
      companion = Object.assign({}, m);
    }
...
<body>
  <div class="uv-toolbar">
    <div style="background:#ff0066;color:#fff;padding:4px 10px;font-weight:900;font-size:11px;letter-spacing:0.5px;border-radius:4px;align-self:center;margin-right:auto">v286 · modo:${modo} · foto1:${m && m.imagen_pieza_url ? '✓' : '✗'} · foto2:${m && m.imagen_ensamble_url ? '✓' : '✗'} · companion:${companion ? '✓' : '✗'}</div>
    <button class="outline" onclick="window.close()">✕ Cerrar</button>
    <button onclick="window.print()">🖨️ Imprimir / Guardar PDF</button>
  </div>
  <div class="uv-page">
