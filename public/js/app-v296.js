...
async function descargarPDF() {
  ...
  try {
    ...
    // Capturar el flyer como imagen de alta calidad
    ...
    // jsPDF letter portrait: 8.5 x 11 in = 612 x 792 pts
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({ orientation: 'portrait', unit: 'pt', format: 'letter' });
    const pageW = 612, pageH = 792;
    const imgW = pageW;
    const imgH = (canvas.height * imgW) / canvas.width;
    // Si imgH > pageH, escalamos proporcionalmente para caber
    if (imgH <= pageH) {
      ...
      pdf.addImage(imgData, 'JPEG', offsetX, 0, finalW, pageH);
    }
