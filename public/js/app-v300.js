...
<style>
  * { box-sizing: border-box; }
  ...
  .uv-hero-fondo { display: block; width: 100%; height: 100%; object-fit: cover; object-position: center center; user-select: none; pointer-events: none; }
  /* Overlay con linear-gradient (blanco → transparente) que simula mask-image y SÍ es capturado por html2canvas */
  .uv-hero-fade {
    position: absolute; top: 0; left: 0; width: 100%; height: 100%;
    background: linear-gradient(to right, #fff 0%, rgba(255,255,255,0.85) 18%, rgba(255,255,255,0) 50%);
    pointer-events: none;
    z-index: 2;
  }
  ...
  * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  ...
  /* @media (max-width: 760px) ELIMINADO INTENCIONALMENTE: rompía window.print() y html2canvas
     porque colapsaba grids a 1 columna cuando el navegador interpretaba viewport como móvil. */
</style>
...
async function descargarPDF() {
  ...
  try {
    ...
    // FIX #3: esperar a que TODAS las imágenes del flyer estén cargadas antes de capturar
    ...
    // Capturar a tamaño natural (sin re-render en iframe). Esto preserva el layout actual.
    const canvas = await html2canvas(flyer, {
      ...
      logging: false,
      // NO usar windowWidth/windowHeight para que use el viewport actual del popup donde ya está bien renderizado.
    });
    // FIX #2: pagination vertical proporcional (escalar por width SIEMPRE, repartir verticalmente si no cabe)
    ...
    if (imgH <= pageH) {
      ...
      // Más alto que la hoja → paginar verticalmente (sin franjas blancas laterales).
      let posicionY = 0;
      while (posicionY < imgH) {
        pdf.addImage(imgData, 'JPEG', 0, -posicionY, imgW, imgH);
        posicionY += pageH;
        if (posicionY < imgH) pdf.addPage();
      }
    }
...
function imprimirFlyer() { window.print(); }
<\/script>
</head>
<body>
