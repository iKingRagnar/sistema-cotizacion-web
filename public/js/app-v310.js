...
async function generarFlyerPDF() {
  ...
  // PDF con dimensiones EXACTAS del flyer (proporción nativa) — sale idéntico al pixel,
  // sin franjas blancas, sin distorsión, sin recortes. Ancho letter, alto proporcional.
  const imgData = canvas.toDataURL('image/jpeg', 0.95);
  const { jsPDF } = window.jspdf;
  const pageW = 612; // 8.5" en pt
...
async function imprimirFlyer() {
  ...
  try {
    // Capturar el flyer como imagen igual que para PDF
    const flyer = document.querySelector('.uv-page');
    await Promise.all([...flyer.querySelectorAll('img')].map(img => {
      if (img.complete && img.naturalWidth > 0) return null;
      return new Promise(r => { img.onload = r; img.onerror = r; setTimeout(r, 3000); });
    }));
    const fullHeight = Math.max(flyer.scrollHeight, flyer.offsetHeight);
    const fullWidth = Math.max(flyer.scrollWidth, flyer.offsetWidth);
    const canvas = await html2canvas(flyer, {
    ...
    const imgData = canvas.toDataURL('image/jpeg', 0.95);
    // Construir HTML con concatenación (evita conflicto de template literal anidado dentro del popup parent)
    // KEY fix: usar height:11in + width:auto (sin object-fit) → respeta aspect ratio nativo, centra con margin auto.
    const printHtml =
      '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Imprimir flyer</title>' +
      '<style>' +
      '* { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; box-sizing: border-box; }' +
      'html, body { margin: 0 !important; padding: 0 !important; background: #fff; text-align: center; }' +
      'img { display: block; margin: 0 auto; max-width: 100%; height: auto; }' +
      '@media print {' +
      '  @page { size: letter portrait; margin: 0 !important; }' +
      '  html, body { width: 8.5in !important; height: 11in !important; margin: 0 !important; padding: 0 !important; overflow: hidden; }' +
      '  img { height: 11in !important; width: auto !important; max-width: 8.5in !important; max-height: 11in !important; margin: 0 auto !important; display: block !important; page-break-inside: avoid !important; object-fit: contain; }' +
      '}' +
      '</style></head><body>' +
      '<img src="' + imgData + '" alt="Flyer">' +
      '<scr' + 'ipt>' +
      'window.addEventListener("load", function(){ setTimeout(function(){ window.print(); }, 500); });' +
      'window.addEventListener("afterprint", function(){ setTimeout(function(){ window.close(); }, 500); });' +
      '</scr' + 'ipt>' +
      '</body></html>';
