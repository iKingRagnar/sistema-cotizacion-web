...
async function generarFlyerPDF() {
  ...
  // PDF en LETTER PORTRAIT EXACTO (612x792 pt) — garantiza que tanto descargar como imprimir
  // muestran el flyer COMPLETO sin recortes. Pequeñas franjas blancas laterales aceptables.
  ...
  if (ratioImg > ratioPage) {
    imgW = pageW; imgH = pageW / ratioImg;
  } else {
    imgH = pageH; imgW = pageH * ratioImg;
  }
...
async function imprimirFlyer() {
  // Approach simplificado: generar el MISMO PDF que descargarPDF (Luis confirmó que se ve bien)
  // y abrirlo en una nueva pestaña con el PDF viewer del navegador. Auto-disparar print.
  // Así garantizamos que imprimir muestra EXACTAMENTE el mismo PDF que se descarga.
  ...
  try {
    ...
    // Disparar print del PDF viewer del navegador (algunos lo bloquean, en ese caso el usuario da Ctrl+P en la pestaña abierta)
