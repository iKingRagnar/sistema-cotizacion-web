...
<div class="cotizacion-doc-preview universal-doc" style="background:#fff;color:#0f172a">
    ...
    const body = `
      <div class="cotz-modal">
        ...
        <section class="cotz-card">
          <h4 class="cotz-card-title"><span class="cotz-step-num">5</span> Imagen y ficha técnica</h4>
          <div class="form-row" style="gap:1rem">
            <div class="form-group" style="flex:1">
              ...
              <div id="cot-prev-imagen" style="margin-top:0.5rem">${cot && cot.imagen_maquina_url ? '<img src="' + escapeHtml(cot.imagen_maquina_url) + '" style="max-width:200px;max-height:140px;border:1px solid #ccc">' : '<span class="muted">—</span>'}</div>
            </div>
            <div class="form-group" style="flex:1">
              ...
              <div id="cot-prev-ficha" style="margin-top:0.5rem;font-size:0.85rem">${cot && cot.ficha_tecnica_url ? '<a href="' + escapeHtml(cot.ficha_tecnica_url) + '" target="_blank"><i class="fas fa-paperclip"></i> Ver ficha actual</a>' : '<span class="muted">—</span>'}</div>
            </div>
          </div>
        </section>
        <section class="cotz-card">
  ...
  function openPdfPreview(kind, id) {
    ...
    if (modalBox) {
      // SIEMPRE centrar al abrir (no usar rect persistido — causaba modal "mocho" del lado derecho).
      try { localStorage.removeItem(PDF_MODAL_RECT_KEY); } catch (_) {}
      const vw = window.innerWidth;
      const vh = window.innerHeight;
      const w = Math.min(1100, Math.max(800, Math.round(vw * 0.9)));
      const h = Math.min(900, Math.max(600, Math.round(vh * 0.85)));
      modalBox.style.position = 'fixed';
      modalBox.style.width = w + 'px';
      modalBox.style.height = h + 'px';
      modalBox.style.left = Math.round((vw - w) / 2) + 'px';
      modalBox.style.top = Math.round((vh - h) / 2) + 'px';
