...
<div class="cotizacion-doc-preview universal-doc" style="background:#fff;color:#0f172a">
  ...
  async function openModalCotizacion(cot) {
    ...
    function _renderImgMaquinaPrev() {
      ...
      if (url) {
        prev.innerHTML = '<div style="display:flex;align-items:flex-start;gap:0.5rem;flex-wrap:wrap">'
          + '<img src="' + url + '" style="max-width:200px;max-height:140px;border:1px solid #ccc;border-radius:6px">'
    ...
    function _renderFichaPrev() {
      ...
      if (url) {
        ...
        const visual = isPdf
          ...
          : '<img src="' + url + '" style="max-width:240px;max-height:160px;border:1px solid #ccc;border-radius:6px">';
        prev.innerHTML = '<div style="display:flex;align-items:flex-start;gap:0.5rem;flex-wrap:wrap">'
          + visual
    ...
    function _renderFichaManual() {
      ...
      body.innerHTML = _fichaArr.map((r, i) => {
        ...
        if (t === 'header') {
          labelCell = `<td colspan="2" style="border:1px solid #ddd;padding:4px;background:rgba(255,210,0,0.15)"><input type="text" data-fk="label" value="${escapeHtml(r.label||'')}" style="width:100%;border:none;background:transparent;padding:4px;font-weight:900;text-transform:uppercase;letter-spacing:0.05em" placeholder="Título de sección — ej: CARACTERÍSTICAS TÉCNICAS"></td>`;
          valueCell = '';
    ...
    function _collectFichaManual() {
      ...
      const out = [];
      rows.forEach((row) => {
        const obj = { type: row.getAttribute('data-row-type') || 'pair' };
