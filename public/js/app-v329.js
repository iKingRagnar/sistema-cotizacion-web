...
<div class="cotizacion-doc-preview universal-doc" style="background:#fff;color:#0f172a">
  ...
  async function openModalCotizacion(cot) {
        ...
        <section class="cotz-card">
          ...
          <div class="form-row" style="gap:1rem">
            ...
            <div class="form-group" style="flex:1;border:2px solid #0a0a0a;border-radius:8px;padding:0.75rem;background:rgba(10,10,10,0.04)">
              <label style="font-weight:800;font-size:0.95rem;color:#0a0a0a">📎 FICHA TÉCNICA <span style="font-weight:400;color:#666;font-size:0.8rem">(va abajo — después de la tabla)</span></label>
              <p class="form-hint" style="margin:0.25rem 0 0.5rem"><i class="fas fa-file-pdf"></i> PDF o imagen con specs técnicos. Se muestra como sección embebida abajo de los conceptos.</p>
              <input type="file" id="cot-ficha-file" accept="application/pdf,image/*">
              <input type="hidden" id="cot-h-ficha-tecnica" value="${escapeHtml((cot && cot.ficha_tecnica_url) || '')}">
              <div id="cot-prev-ficha" style="margin-top:0.5rem;font-size:0.85rem"></div>
              <p class="form-hint" style="margin:0.5rem 0 0;font-size:0.78rem;color:#666"><i class="fas fa-info-circle"></i> Si hay imagen cargada, ésta tiene preferencia sobre la tabla manual. Elimínala con el botón para que se muestre la tabla.</p>
            </div>
          </div>
    ...
    function _renderFichaPrev() {
      const prev = qm('#cot-prev-ficha');
      const hid = qm('#cot-h-ficha-tecnica');
      if (!prev || !hid) return;
      const url = (hid.value || '').trim();
      if (url) {
        const isPdf = url.toLowerCase().startsWith('data:application/pdf') || /\.pdf(\?|$)/i.test(url);
        const visual = isPdf
          ? '<a href="' + url + '" target="_blank" style="display:inline-block;padding:0.5rem 0.75rem;background:#0a0a0a;color:#FFD200;border-radius:6px;text-decoration:none"><i class="fas fa-file-pdf"></i> Ver PDF cargado</a>'
          : '<div style="width:100%;max-width:420px;background:#f5f5f5;border:1px solid #ccc;border-radius:6px;padding:6px;display:flex;align-items:center;justify-content:center"><img src="' + url + '" style="max-width:100%;max-height:220px;object-fit:contain;display:block;border-radius:4px"></div>';
        prev.innerHTML = '<div style="display:flex;align-items:center;gap:0.75rem;flex-wrap:wrap">'
          + visual
          + '<button type="button" class="btn small danger" id="cot-ficha-del"><i class="fas fa-trash"></i> Quitar ficha</button>'
          + '</div>';
        const btn = qm('#cot-ficha-del');
        if (btn) btn.addEventListener('click', () => {
          hid.value = '';
          const inp = qm('#cot-ficha-file'); if (inp) inp.value = '';
          _renderFichaPrev();
        });
      } else {
        prev.innerHTML = '<span class="muted" style="font-style:italic">— Sin imagen/PDF (se usará tabla manual si tiene filas) —</span>';
      }
    }
    _renderFichaPrev();
    const _fichaFile = qm('#cot-ficha-file');
    if (_fichaFile) {
      _fichaFile.addEventListener('change', async () => {
        const f = _fichaFile.files && _fichaFile.files[0];
        if (!f) return;
        try {
          const dataUrl = await _readFileAsDataUrl(f);
          const hid = qm('#cot-h-ficha-tecnica');
    ...
    qm('#cotz-save').onclick = async () => {
      ...
      const payload = {
        ...
        ficha_tecnica_url: (qm('#cot-h-ficha-tecnica')?.value || '').trim() || null,
