...
<div class="cotizacion-doc-preview universal-doc" style="background:#fff;color:#0f172a">
  ...
  function renderMantenimientoGarantiaCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      // Chips de técnicos ocupados con candado y slot
      const chips = asigns.slice(0, 3).map(a => {
        const lbl = SLOT_LABEL[a.slot] || (a.slot || '—');
        const tt = `${a.tecnico} · ${lbl} · ${a.folio || ''} · ${a.cliente_nombre || a.razon_social || ''}`;
        return `<button type="button" class="cal-tec-chip" data-asign-id="${a.id}" title="${escapeHtml(tt)}"><i class="fas fa-lock"></i> <strong>${escapeHtml(a.tecnico || '')}</strong> · <span class="cal-tec-slot">${lbl}</span></button>`;
      }).join('');
  ...
  function openModalAgendaDia(dateIso, items, asigns) {
    ...
    const SLOT_LBL = { madrugada: '🌙 Madrugada (00-06)', manana: '🌅 Mañana (06-12)', tarde: '☀️ Tarde (12-18)', noche: '🌃 Noche (18-24)' };
    let html = `<div style="margin-bottom:0.75rem"><strong style="font-size:1.05rem">${escapeHtml(pretty)}</strong></div>`;
    if (asigns && asigns.length) {
      html += '<h4 style="margin:0.5rem 0;font-size:0.95rem"><i class="fas fa-user-clock"></i> Técnicos asignados (' + asigns.length + ')</h4>';
      html += '<div style="display:flex;flex-direction:column;gap:0.5rem;margin-bottom:0.75rem">';
      asigns.forEach(a => {
        const lbl = SLOT_LBL[a.slot] || (a.slot || 'Sin slot');
        html += `<button type="button" class="agenda-detalle-card" data-open-asign="${a.id}" style="text-align:left;background:#fffbe6;border:1px solid #FFD200;border-radius:8px;padding:0.6rem 0.8rem;cursor:pointer;display:flex;flex-direction:column;gap:0.2rem">
          <div style="font-weight:800"><i class="fas fa-lock" style="color:#dc2626"></i> ${escapeHtml(a.tecnico||'—')} <span style="color:#666;font-weight:600">· ${escapeHtml(lbl)}</span></div>
          <div style="font-size:0.85rem;color:#475569">${escapeHtml(a.folio || '')} · ${escapeHtml(a.cliente_nombre || a.razon_social || '—')} · ${escapeHtml(a.maquina_nombre || '—')}</div>
          <div style="font-size:0.78rem;color:#94a3b8">Click para ver detalle completo</div>
        </button>`;
      });
      html += '</div>';
    }
    if (items && items.length) {
      html += '<h4 style="margin:0.5rem 0;font-size:0.95rem"><i class="fas fa-wrench"></i> Mantenimientos (' + items.length + ')</h4>';
  ...
  function openModalAsignacionDetalle(a) {
    ...
    const html = `
      <div style="display:flex;flex-direction:column;gap:0.5rem">
        <div style="background:#0a0a0a;color:#FFD200;padding:0.5rem 0.8rem;border-radius:6px;font-weight:800">
          <i class="fas fa-lock"></i> ${escapeHtml(a.tecnico||'—')} · ${escapeHtml(SLOT_LBL[a.slot]||a.slot||'Sin slot')}
        </div>
        ...
        <div><strong>Máquina:</strong> ${escapeHtml(a.maquina_nombre || '—')} ${a.numero_maquina ? '· '+escapeHtml(a.numero_maquina) : ''}</div>
        <div><strong>Actividad:</strong> ${escapeHtml(tipoLbl)}</div>
        <div><strong>Estatus:</strong> ${escapeHtml(a.estatus || '—')}</div>
        ${a.descripcion ? `<div style="margin-top:0.4rem"><strong>Descripción:</strong><br>${escapeHtml(a.descripcion)}</div>` : ''}
        ${a.notas ? `<div><strong>Notas:</strong><br>${escapeHtml(a.notas)}</div>` : ''}
      </div>
