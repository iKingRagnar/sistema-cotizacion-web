...
<div class="cotizacion-doc-preview universal-doc" style="background:#fff;color:#0f172a">
  ...
  function renderMantenimientoGarantiaCalendar() {
    ...
    wrap.querySelectorAll('.cal-tec-chip[data-asign-id]').forEach(chip => {
      chip.addEventListener('click', (e) => {
        ...
        if (asign) openModalAsignacionDetalle(asign);
      });
    });
  }
  ...
  function openModalAgendaDia(dateIso, items, asigns) {
    ...
    // Header del modal
    let html = `<div style="margin-bottom:0.6rem;padding:0.5rem 0.8rem;background:linear-gradient(90deg,#0a0a0a,#1f2937);color:#FFD200;border-radius:8px;border:1px solid #FFD200">
      <i class="fas fa-calendar-day"></i> <strong style="font-size:1.02rem">${escapeHtml(pretty)}</strong>
      <span style="float:right;font-size:0.8rem;background:#FFD200;color:#0a0a0a;padding:2px 8px;border-radius:6px;font-weight:800">${asigns.length} asignación(es)</span>
    </div>`;
    ...
    SLOTS_ORDER.forEach(s => {
      ...
      if (!arr.length) {
        html += `<div class="agenda-slot-block libre">
          ...
          <div class="agenda-slot-info" style="color:#10b981;font-weight:700">Sin asignaciones</div>
        </div>`;
      } else {
        // Si hay varias asignaciones en el mismo slot, las apila
        const tecnicosHtml = arr.map(a => `<div class="agenda-slot-tecnico" data-open-asign="${a.id}" style="cursor:pointer">
        ...
        const detalleHtml = arr.map(a => `<div class="agenda-slot-info" data-open-asign="${a.id}" style="cursor:pointer;border-top:1px dashed rgba(220,38,38,0.25);padding-top:3px;margin-top:2px">
          <strong>${escapeHtml(a.folio||'')}</strong> · ${escapeHtml(a.cliente_nombre || a.razon_social || '—')}
        </div>`).join('');
    ...
    html += '</div>';
    // Asignaciones sin slot definido
    if (bySlot.sinslot.length) {
      html += '<div style="margin:0.5rem 0;padding:8px 10px;border:1px dashed #94a3b8;border-radius:8px;background:#f8fafc;font-size:0.82rem">';
      html += '<strong style="color:#64748b">⚠️ Asignaciones sin slot definido (edita para asignar horario):</strong>';
      bySlot.sinslot.forEach(a => {
        html += `<div data-open-asign="${a.id}" style="cursor:pointer;padding:4px 0;border-top:1px solid #e2e8f0;margin-top:4px">
          <i class="fas fa-lock" style="color:#dc2626"></i> <strong>${escapeHtml(a.tecnico||'—')}</strong> · ${escapeHtml(a.folio||'')} · ${escapeHtml(a.cliente_nombre || a.razon_social || '—')}
        </div>`;
      });
      html += '</div>';
    }
    if (items && items.length) {
      html += '<h4 style="margin:0.75rem 0 0.4rem;font-size:0.9rem;color:#0f172a"><i class="fas fa-wrench"></i> Mantenimientos programados (' + items.length + ')</h4>';
      html += '<ul style="margin:0;padding-left:1.2rem;font-size:0.85rem">';
      items.forEach(ev => {
    ...
    if ((!asigns || !asigns.length) && (!items || !items.length)) {
      html += '<p class="muted" style="text-align:center;padding:1rem">Sin asignaciones ni mantenimientos para este día.</p>';
    }
    ...
    document.querySelectorAll('[data-open-asign]').forEach(b => {
      b.addEventListener('click', () => {
        ...
        const a = (agendaAsignacionesCache || []).find(x => String(x.id) === String(id));
        if (a) openModalAsignacionDetalle(a);
      });
    });
  }
