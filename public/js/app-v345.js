...
<div class="cotizacion-doc-preview universal-doc" style="background:#fff;color:#0f172a">
  ...
  async function loadAgendaAsignaciones(ym) {
    try {
      ...
      const raw = await fetchJson(API + `/agenda/mes?year=${Y}&month=${M}`);
      agendaAsignacionesCache = Array.isArray(raw) ? raw : [];
    } catch (_) { agendaAsignacionesCache = []; }
  }
  async function loadMantenimientoGarantia() {
    ...
    try {
      ...
      const miEl = qs('#mant-gar-month');
      await loadAgendaAsignaciones(miEl && miEl.value ? miEl.value : null);
  ...
  function renderMantenimientoGarantiaCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      // Chips de técnicos ocupados con candado vial y slot
      const chips = asigns.slice(0, 3).map(a => {
        const lbl = SLOT_LABEL[a.slot] || (a.slot ? a.slot : 'Sin slot');
        const tt = `${a.tecnico} · ${lbl} · ${a.folio || ''} · ${a.cliente_nombre || a.razon_social || ''}`;
        return `<button type="button" class="cal-tec-chip" data-asign-id="${a.id}" title="${escapeHtml(tt)}"><span class="lock-badge"><i class="fas fa-lock"></i></span> <strong>${escapeHtml(a.tecnico || '')}</strong> <span class="cal-tec-slot">${lbl}</span></button>`;
      }).join('');
  ...
  function openModalAgendaDia(dateIso, items, asigns) {
    ...
    // Header del modal
    let html = `<div style="margin-bottom:0.7rem;padding:0.6rem 0.9rem;background:linear-gradient(90deg,#0a0a0a,#1f2937);color:#FFD200 !important;border-radius:8px;border:1px solid #FFD200">
      <i class="fas fa-calendar-day"></i> <strong style="font-size:1.05rem;color:#FFD200 !important">${escapeHtml(pretty)}</strong>
      <span style="float:right;font-size:0.82rem;background:#FFD200;color:#0a0a0a !important;padding:3px 10px;border-radius:8px;font-weight:900">${asigns.length} asignación(es)</span>
    </div>`;
    ...
    const renderAsignFull = (a) => {
      ...
      // Slots libres del mismo técnico ese día
      ...
      const libresHtml = libres.length
        ...
        : '<span style="color:#dc2626 !important;font-weight:800">Sin slots libres este día</span>';
      return `<div style="background:#fff !important;border:1.5px solid #dc2626;border-radius:10px;padding:10px 12px;margin-top:6px;box-shadow:0 2px 8px rgba(220,38,38,0.12)">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
          <span style="width:24px;height:24px;border-radius:50%;background:linear-gradient(135deg,#dc2626,#991b1b);color:#fff !important;display:inline-flex;align-items:center;justify-content:center;font-size:0.75rem;box-shadow:0 2px 4px rgba(220,38,38,0.5)"><i class="fas fa-lock"></i></span>
          <strong style="font-size:1rem;color:#0a0a0a !important">${escapeHtml(a.tecnico||'—')}</strong>
          ${m ? `<span style="margin-left:auto;background:#0a0a0a !important;color:#FFD200 !important;padding:3px 9px;border-radius:6px;font-weight:800;font-size:0.78rem">${m.icon} ${m.titulo} · ${m.hora}</span>`
              : `<span style="margin-left:auto;background:#fbbf24 !important;color:#0a0a0a !important;padding:3px 9px;border-radius:6px;font-weight:800;font-size:0.78rem">⚠️ Sin slot — edita para asignar horario</span>`}
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 12px;font-size:0.85rem;color:#0a0a0a !important">
          <div style="color:#0a0a0a !important"><strong style="color:#0a0a0a !important">Folio:</strong> ${escapeHtml(a.folio || '#'+a.id)}</div>
          <div style="color:#0a0a0a !important"><strong style="color:#0a0a0a !important">Fecha:</strong> ${escapeHtml(a.fecha || '—')}</div>
          <div style="color:#0a0a0a !important"><strong style="color:#0a0a0a !important">Cliente:</strong> ${escapeHtml(a.cliente_nombre || a.razon_social || '—')}</div>
          <div style="color:#0a0a0a !important"><strong style="color:#0a0a0a !important">Estatus:</strong> <span style="background:#fef3c7 !important;color:#92400e !important;padding:1px 7px;border-radius:5px;font-weight:700;font-size:0.74rem">${escapeHtml(a.estatus || '—')}</span></div>
          <div style="grid-column:1/-1;color:#0a0a0a !important"><strong style="color:#0a0a0a !important">Máquina:</strong> ${escapeHtml(a.maquina_nombre || '—')} ${a.numero_maquina ? '· '+escapeHtml(a.numero_maquina) : ''}</div>
          <div style="grid-column:1/-1;color:#0a0a0a !important"><strong style="color:#0a0a0a !important">Actividad:</strong> ${escapeHtml(tipoLbl)}</div>
        </div>
        ${a.descripcion ? `<div style="margin-top:6px;background:#fffbe6 !important;border-left:3px solid #FFD200;padding:5px 9px;border-radius:4px;font-size:0.82rem;color:#0a0a0a !important"><strong style="color:#0a0a0a !important">Descripción:</strong> ${escapeHtml(a.descripcion)}</div>` : ''}
        ${a.notas ? `<div style="margin-top:4px;background:#fef9c3 !important;border-left:3px solid #f59e0b;padding:5px 9px;border-radius:4px;font-size:0.82rem;color:#0a0a0a !important"><strong style="color:#0a0a0a !important">Notas:</strong> ${escapeHtml(a.notas)}</div>` : ''}
        <div style="margin-top:8px;background:#ecfdf5 !important;border:1px dashed #10b981;border-radius:6px;padding:7px 10px">
          <div style="font-weight:800;color:#065f46 !important;font-size:0.78rem;margin-bottom:4px"><i class="fas fa-check-circle"></i> ${escapeHtml(a.tecnico||'Este técnico')} está LIBRE en:</div>
          <div style="line-height:2">${libresHtml}</div>
        </div>
    ...
    if (asigns && asigns.length) {
      html += '<h4 style="margin:0.75rem 0 0.3rem;font-size:0.92rem;color:#FFD200 !important;font-weight:800"><i class="fas fa-user-clock"></i> Detalle de asignaciones</h4>';
      asigns.forEach(a => { html += renderAsignFull(a); });
    }
    if (items && items.length) {
      html += '<h4 style="margin:0.75rem 0 0.3rem;font-size:0.92rem;color:#FFD200 !important;font-weight:800"><i class="fas fa-wrench"></i> Mantenimientos programados (' + items.length + ')</h4>';
      html += '<ul style="margin:0;padding-left:1.2rem;font-size:0.85rem;color:#fff !important">';
      items.forEach(ev => {
        html += `<li style="color:#fff !important">${escapeHtml(ev.razon_social || '—')} · ${escapeHtml(ev.modelo_maquina || '—')}</li>`;
      });
      html += '</ul>';
    }
    if ((!asigns || !asigns.length) && (!items || !items.length)) {
      html += '<p style="text-align:center;padding:1rem;color:#94a3b8 !important">Sin asignaciones ni mantenimientos para este día.</p>';
    }
    ...
    // Click en "Editar reporte" -> abrir editor del reporte
    document.querySelectorAll('[data-open-asign]').forEach(b => {
