...
<div class="cotizacion-doc-preview universal-doc" style="background:#fff;color:#0f172a">
  ...
  function openModalAgendaDia(dateIso, items, asigns) {
    ...
    // Header del modal — wrapper light que ignora dark theme
    let html = `<div class="agenda-light-modal">`;
    html += `<div class="ag-dark-text" style="margin-bottom:10px;padding:10px 14px;background:linear-gradient(90deg,#0a0a0a 0%,#1f2937 70%,#2a1f00 100%);border-radius:8px;border:1.5px solid #FFD200;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
      <i class="fas fa-calendar-day"></i>
      <strong style="font-size:1.05rem">${escapeHtml(pretty)}</strong>
      <span style="margin-left:auto;background:#FFD200;padding:4px 12px;border-radius:8px;font-weight:900;font-size:0.85rem;color:#0a0a0a">${asigns.length} asignación(es)</span>
    </div>`;
    ...
    const renderAsignFull = (a) => {
      ...
      return `<div style="background:#ffffff;border:1.5px solid #dc2626;border-radius:10px;padding:12px;margin-top:8px;box-shadow:0 2px 8px rgba(220,38,38,0.12)">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;flex-wrap:wrap">
          <span style="width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#dc2626,#991b1b);display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 4px rgba(220,38,38,0.5)"><i class="fas fa-lock" style="color:#ffffff;font-size:0.78rem"></i></span>
          <strong style="font-size:1.02rem">${escapeHtml(a.tecnico||'—')}</strong>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:5px 14px;font-size:0.88rem">
        ...
        ${a.descripcion ? `<div style="margin-top:8px;background:#fffbe6;border-left:4px solid #FFD200;padding:6px 10px;border-radius:4px;font-size:0.86rem"><strong>Descripción:</strong> ${escapeHtml(a.descripcion)}</div>` : ''}
        ${a.notas ? `<div style="margin-top:5px;background:#fef9c3;border-left:4px solid #f59e0b;padding:6px 10px;border-radius:4px;font-size:0.86rem"><strong>Notas:</strong> ${escapeHtml(a.notas)}</div>` : ''}
        <div style="margin-top:10px;background:#ecfdf5;border:1.5px dashed #10b981;border-radius:8px;padding:8px 12px">
          <div style="font-weight:900;font-size:0.86rem;margin-bottom:6px;color:#065f46 !important"><i class="fas fa-check-circle"></i> ${escapeHtml(a.tecnico||'Este técnico')} está LIBRE en:</div>
          <div style="line-height:2.2">${libresHtml}</div>
        </div>
