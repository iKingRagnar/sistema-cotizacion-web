...
(function premiumBg() {
  ...
  /** Tema industrial (nano): sin parallax ni spotlight — el fondo es la foto, no el gradiente animado de premium.css. */
  function themeIndustrial() {
    return document.body && document.body.classList.contains('theme-industrial');
  }

  function clearPremiumPointerCssVars() {
    document.documentElement.style.removeProperty('--pmx');
    document.documentElement.style.removeProperty('--pmy');
    document.querySelectorAll('table.data-table tbody').forEach(function (tbody) {
      tbody.style.removeProperty('--tx');
      tbody.style.removeProperty('--ty');
    });
  }
  ...
  /* ─── Bootstrap ─────────────────────────────────────────── */
  function boot() {
    /* Sin parallax, spotlight ni partículas: UI estática (html.no-motion + no-motion.css). */
    clearPremiumPointerCssVars();
    initMobileSidebar();
  }

  function bootAfterIdleTheme() {
    boot();
    clearPremiumPointerCssVars();
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bootAfterIdleTheme);
  } else {
    bootAfterIdleTheme();
  }
